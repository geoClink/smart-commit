...
def build_chunk_summary_prompt(chunk_diff, chunk_index, total_chunks):
    return textwrap.dedent(
        ...
        - Keep each bullet to one short line (under 80 characters). No multi-line bullets.
...
def build_merge_prompt(all_bullets, developer_context=None, previous_message=None, feedback_history=None, ticket=None):
    ...
    return textwrap.dedent(
        ...
        - Each bullet is a concise completed action (e.g. "Added X", "Removed Y", "Fixed Z"). Keep it under 80 characters, one line only.
...
if __name__ == "__main__":
    ...
    if args.provider == "mlx":
        try:
            ...
            print("MLX unavailable, falling back to Groq...")
            groq_key = args.groq_key or os.environ.get("GROQ_API_KEY")
            if groq_key:
                try:
                    respond = setup_groq_provider(groq_key)
                    chunking = False
                    ...
                    try:
                        ...
                        print("Ollama unavailable, falling back to Apple Intelligence...")
                        respond = setup_apple_provider()
                        chunking = True
                        provider_label = "Generated by Apple Intelligence"
            else:
                print("No Groq key found, falling back to Ollama...")
                try:
                    respond = setup_ollama_provider(args.ollama_model)
                    chunking = False
                    provider_label = f"Generated by Ollama · {args.ollama_model}"
